...
app.get('/login-deezer', (_req, res) => {
  res.redirect(
    `https://connect.deezer.com/oauth/auth.php?${qs.stringify({
      app_id: process.env.DEEZER_CLIENT_ID,
      perms:
        'basic_access,email,listening_history,offline_access,manage_library,manage_community,delete_library',
