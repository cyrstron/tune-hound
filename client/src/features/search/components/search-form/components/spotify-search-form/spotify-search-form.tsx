...
class SpotifySearchFormComponent extends Component<DeezerSearchFormProps, DeezerSearchFormState> {
  ...
  render() {
    const { type, query, market, includeExternal } = this.props.searchParams;
    ...
    return (
      <div>
        ...
        <button onClick={this.onToggleAdvance}>Advanced {isAdvancedOpen ? '⇑' : '⇓'}</button>
        {isAdvancedOpen && (
          <>
            ...
            <div>
              <label>
                Market:{' '}
                <input value={market || ''} onChange={this.onMarketChange} disabled={!query} />
              </label>
            </div>
            <div>
              <label>
