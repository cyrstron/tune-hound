...
const DeezerItemDetailsComponent: FC<DeezerItemDetailsProps> = ({ id, item, className }) => {
  switch (item.type) {
    case 'track':
      return <DeezerTrackDetails id={id} track={item} className={cx(className)} />;
    case 'album':
      return <DeezerAlbumDetails id={id} album={item} className={cx(className)} />;
    case 'playlist':
      return <DeezerPlaylistDetails id={id} playlist={item} className={cx(className)} />;
    case 'artist':
      return <DeezerArtistDetails id={id} artist={item} className={cx(className)} />;
    default:
      return null;
  }
};
