...
const SpotifyItemDetailsComponent: FC<SpotifyItemDetailsProps> = ({ id, item, className }) => {
  switch (item.type) {
    case 'track':
      return <SpotifyTrackDetails id={id} track={item} className={cx(className)} />;
    case 'album':
      return <SpotifyAlbumDetails id={id} album={item} className={cx(className)} />;
    case 'playlist':
      return <SpotifyPlaylistDetails id={id} playlist={item} className={cx(className)} />;
    case 'artist':
      return <SpotifyArtistDetails id={id} artist={item} className={cx(className)} />;
    default:
      return null;
  }
};
